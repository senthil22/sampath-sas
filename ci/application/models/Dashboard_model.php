...
class Dashboard_model extends CI_Model {
	...
	public function get_allpurchase() {
		$result = $this->db->query("SELECT purchase.*, products.prod_name FROM purchase LEFT JOIN products on products.prod_id = purchase.prod_id")->result_array();;
		return $result;
	}
	...
	public function get_allsales() {
		$result = $this->db->query("SELECT sales.*, products.prod_name FROM sales LEFT JOIN products on products.prod_id = sales.prod_id")->result_array();;
		return $result;
	}
	...
	public function reportsbydate($table=null, $sdate=null, $edate=null) {
		if(!empty($table)) {
			$this->db->select('*');
			if(!empty($sdate)) {
	...
	public function reportstotal($table=null, $sdate=null, $edate=null) {
		if(!empty($table)) {
	...
	public function buyerstotal($table=null, $sdate=null, $edate=null) {
		if(!empty($table)) {
			$this->db->select_sum('total_amt', 'total')
					 ->select_sum('paid_amt', 'paidtotal');
			if(!empty($sdate)) {
				$this->db->where('date >= ', date($sdate));
			}
			if(!empty($edate)) {
